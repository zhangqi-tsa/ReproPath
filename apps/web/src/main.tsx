...
function App(): React.JSX.Element {
  ...
  const { session, events, actions, signals, findings, detectionStats, agentRuns, frame, error: restoreError, connection, acknowledge, control, controlState } = useSession(id);
  ...
  return <main>
    <header><a className="brand" href="/" aria-label="ReproPath 首页">R<span>↗</span></a><div><h1>ReproPath</h1><p>Agent Driver Foundation · Milestone 2.0</p></div><span className="milestone">LOCAL RUNTIME</span></header>
    ...
    {session ? <>
      ...
      </section><AgentPanel key={`agent-${session.id}`} sessionId={session.id} running={session.status==='running'} humanOwned={['HUMAN CONTROL','CONTROLLED BY OTHER'].includes(controlState.mode)} runs={agentRuns}/><Findings key={session.id} sessionId={session.id} findings={findings} signals={signals} stats={detectionStats} actions={actions} events={events} /><Actions actions={actions} events={events} /><Timeline events={events} />
